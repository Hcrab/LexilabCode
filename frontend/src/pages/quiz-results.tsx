      ...
      if (router.isReady) {
        ...
        if (user && resultData) {
          try {
            ...
            const errorRes = await fetch(`/api/bookmarks?username=${user.username}&type=error_question&result_id=${resultData.id}`);
            if (errorRes.ok) {
  ...
  const recalcTotals = (updated: any) => {
    ...
    qs.forEach((q: any) => {
      if (q.type === 'fill-in-the-blank') {
        totalSum += FILL_BLANK_POINTS;
        correctSum += q.correct ? FILL_BLANK_POINTS : 0;
      } else if (q.type === 'sentence') {
        totalSum += SENTENCE_POINTS;
